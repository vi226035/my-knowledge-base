...
> 概念性的深度学习问题（为什么需要激活函数、反向传播怎么算等）见 [[ML/一些疑惑解答]]。
...
> 详细解释见 [[ML/一些疑惑解答#Q8：`loss → zero_grad → backward → step` 到底在干什么？]]
...
需要继续深入的概念（反向传播数学、激活函数原理、LSTM 机制等）见 [[ML/一些疑惑解答]]。
